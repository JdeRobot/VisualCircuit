...
angular.module('visualcircuit')
    .service('blocks', function (joint,
        ...
        function newBasicInput(callback) {
            ...
            var strPrompt = gettextCatalog.getString('Enter the input blocks');
            var inputValue = 'in';
            var promptCallback = function (evt, values) {
                ...
                for (var l in labels) {
                    if (labels[l]) {
                        ...
                        if (portInfo) {
                            ...
                            evt.cancel = true;
                            alertify.warning(gettextCatalog.getString('Wrong block name {{name}}', {
                                name: labels[l]
                            }));
                            return;
                        }
            ...
            //////////////////////////////////////////////////////////////////////
            utils.inputprompt(strPrompt, inputValue, promptCallback);
                    ...
                    for (i in inPorts) {
                        if (inPorts[i]) {
                            ...
                            if (inPortInfo && inPortInfo.name) {
                                ...
                                evt.cancel = true;
                                alertify.warning(gettextCatalog.getString('Wrong port name {{name}}', {
                                    name: inPorts[i]
                                }));
                                return;
                            }
                    ...
                    for (p in params) {
                        if (params[p]) {
                            ...
                            if (paramInfo) {
                                ...
                                evt.cancel = true;
                                alertify.warning(gettextCatalog.getString('Wrong parameter name {{name}}', {
                                    name: params[p]
                                }));
                                return;
                            }
        ...
        function newGeneric(type, block, callback) {
            ...
            if (block &&
                ...
                alertify.error(gettextCatalog.getString('Wrong block format: {{type}}', {
                    type: type
                }), 30);
            }
        }
        ...
        function editBasicOutput(cellView, callback) {
            ...
            utils.inputcheckboxprompt([
                ...
                block.data.name + (block.data.range || ''), !block.data.virtual
            ],
            function (evt, values) {
                var oldSize, newSize, offset = 0;
