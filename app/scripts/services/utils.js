...
angular.module('visualcircuit')
    .service('utils', function ($rootScope, gettextCatalog, common, _package, window,
        nodeFs, nodeFse, nodePath, nodeChildProcess, nodeExtract,nodeZlib, nodeOnline, 
        nodeGlob, nodeSha1, nodeCP, nodeGetOS, nodeLangInfo, SVGO) {
        ...
        this.inputprompt = function (message, value, callback) {
            var content = [];
            content.push('<div>');
            content.push('  <p>' + message + '</p>');
            content.push('  <input class="ajs-input" id="inputPrompt" type="text" value="' + value.toString() + '"/>');
            content.push('</div>');

            alertify.confirm(content.join('\n'))
                .set('onok', function (evt) {
                    var values = [];
                    values.push($('#inputPrompt').val());
                    if (callback) {
                        callback(evt, values);
                    }
                })
