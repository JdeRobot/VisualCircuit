...
from wires.structure_str import MD
...
class Wire_Read:
    ...
    def get(self):
        ...
        self.sem.acquire()
        f = np.ndarray(shape=(md.shape_0), dtype='<U6', buffer=self.shm_buf)
        self.sem.release()
        return f
...
class Wire_Write:
    ...
    def add(self, frame: np.ndarray):
        byte_size = frame.size * frame.itemsize #frame.nbytes
        ...
        self.count += 1
        md = MD(frame.shape[0], byte_size, self.count)
        self.sem.acquire()
