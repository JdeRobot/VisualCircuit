    ...
    for wire in wires:
        ...
        if source["block"] in parameters:
            ...
            if source["name"] in block_data[source["block"]]["outputs"]:
                ...
                import time
                # Generate a unique wire name using timestamp and random chars
                # This prevents collisions with stale shared memory from previous runs on Windows
                wire_name = "WIRE_" + str(int(time.time() * 1000))[-6:] + "_" + "".join(
