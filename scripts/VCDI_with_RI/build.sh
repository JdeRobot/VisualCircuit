...
Help()
{
   ...
   echo "  -vc,--visual-circuit  <value>  Branch of Visual Circuit.                     Default: master"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -F | --force-no-cache)
            ...
            ;;
        -h | --help) # display Help
            echo "Generates Visual Circuit image"
            echo
            Help
            exit 0
            ;;
...
echo "VISUAL_CIRCUIT:-------------:$VISUAL_CIRCUIT"
...
docker build --no-cache -f $DOCKERFILE \
  --build-arg VISUAL_CIRCUIT=$VISUAL_CIRCUIT \
  --build-arg ROBOTICS_INFRASTRUCTURE=$ROBOTICS_INFRASTRUCTURE \
  --build-arg RAM=$RAM \
  --build-arg ROS_DISTRO=$ROS_DISTRO \
  --build-arg IMAGE_TAG=$IMAGE_TAG \
  -t jderobot/visual-circuit:$IMAGE_TAG .